...
// Removed HlsTrack class to use JSObject directly for compatibility
...
extension HlsExtension on Hls {
  ...
  // Track APIs using JSArray<JSObject>
...
class _Html5VideoPlayerState extends State<Html5VideoPlayer> {
  ...
  void _loadStream(String src) {
     ...
     if (src.toLowerCase().contains('.m3u8') && _isHlsSupported()) {
         debugPrint('Using HLS.js for $src');
         _initHls(src);
     } else {
         debugPrint('Using Native Video for $src');
  ...
  void _updateTracks() {
    ...
    try {
      ...
      debugPrint('Error updating tracks: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    if (!kIsWeb) return const Center(child: Text('HTML5 Player only works on web'));
